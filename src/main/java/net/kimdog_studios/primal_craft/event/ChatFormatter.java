    ...
    private static void formatAndBroadcast(ServerPlayerEntity sender, SignedMessage signed) {
        ...
        server.getPlayerManager().getPlayerList().forEach(p ->
            net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.send(p,
                new net.kimdog_studios.primal_craft.network.ChatAnimatedPayload(role, sender.getName().getString(), messageToSend))
        );

        // Play sounds
        sender.playSound(net.kimdog_studios.primal_craft.sound.ModSounds.CHAT_SEND, 0.7f, 1.0f);
        server.getPlayerManager().getPlayerList().forEach(p -> {
            if (p != sender) {
                p.playSound(net.kimdog_studios.primal_craft.sound.ModSounds.CHAT_RECEIVE, 0.5f, 1.0f);
            }
        });
        ...
        net.kimdog_studios.primal_craft.event.TypingIndicatorHandler.stopTyping(sender);
    }
