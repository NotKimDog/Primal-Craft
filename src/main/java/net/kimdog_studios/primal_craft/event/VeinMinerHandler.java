...
public class VeinMinerHandler {
    ...
    public static void register() {
        PrimalCraft.LOGGER.info("⛏️  [VEINMINER] Registering VeinMiner event handler");
        PrimalCraft.LOGGER.debug("   ├─ Max blocks per vein: {}", MAX_BLOCKS);
        PrimalCraft.LOGGER.debug("   ├─ Max search range: {}", MAX_RANGE);
        PrimalCraft.LOGGER.debug("   ├─ Particle effects: {}", ENABLE_PARTICLES ? "ENABLED" : "DISABLED");
        PrimalCraft.LOGGER.debug("   ├─ Cascade breaking: {}", ENABLE_CASCADE ? "ENABLED" : "DISABLED");
        PrimalCraft.LOGGER.debug("   └─ Registered ore types: {}", ORE_BLOCKS.size());
        ...
        PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
            ...
            veinMinesTriggered++;
            PrimalCraft.LOGGER.debug("⛏️  [VEINMINE] Event #{} - {} broke {}",
            ...
            if (veinBlocks.isEmpty()) {
                PrimalCraft.LOGGER.trace("   └─ No adjacent ore blocks found (single block)");
                return;
            }

            PrimalCraft.LOGGER.debug("   ├─ Vein size: {} blocks (including initial)", veinBlocks.size() + 1);
            ...
            PrimalCraft.LOGGER.trace("   ├─ Total vein cost: {}", String.format("%.2f", totalVeinCost));
            ...
            PrimalCraft.LOGGER.info("VeinMiner: Breaking {} blocks for {}", veinBlocks.size(), serverPlayer.getName().getString());
                ...
                timer.schedule(new java.util.TimerTask() {
                    ...
                    public void run() {
                        world.getServer().execute(() -> {
                            ...
                            PrimalCraft.LOGGER.info(" Breaking block {} at {}, {}, {}", index + 1, blockPos.getX(), blockPos.getY(), blockPos.getZ());
    ...
    private static void applyBreaksInstantly(ServerPlayerEntity player, ServerWorld world, List<BlockPos> positions, BlockPos originPos, ItemStack tool) {
        PrimalCraft.LOGGER.info(" Breaking {} blocks instantly...", positions.size());
    ...
    private static void spawnParticles(ServerWorld world, BlockPos pos, BlockState state) {
        ...
        try {
            ...
            PrimalCraft.LOGGER.debug("Particle effect error: {}", e.getMessage());
        }
    }
    ...
    private static void spawnActivationAnimation(ServerWorld world, BlockPos pos) {
        try {
            ...
            PrimalCraft.LOGGER.debug("Activation animation error: {}", e.getMessage());
        }
    }
    ...
    private static void spawnCompletionAnimation(ServerWorld world, BlockPos pos, int blocksDestroyed) {
        try {
            ...
            PrimalCraft.LOGGER.debug("Completion animation error: {}", e.getMessage());
        }
    }
    ...
    private static void playSoundEffect(ServerWorld world, BlockPos pos) {
        try {
            ...
            PrimalCraft.LOGGER.debug("Sound effect error: {}", e.getMessage());
        }
    }
