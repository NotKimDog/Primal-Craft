package net.kimdog_studios.primal_craft.item.custom;

import net.kimdog_studios.primal_craft.entity.custom.TomahawkProjectileEntity;
import net.kimdog_studios.primal_craft.util.ItemWeightSystem;
import net.kimdog_studios.primal_craft.util.StaminaSystem;
import net.minecraft.entity.player.PlayerEntity;
import net.minecraft.item.Item;
import net.minecraft.item.ItemStack;
import net.minecraft.server.network.ServerPlayerEntity;
import net.minecraft.sound.SoundCategory;
import net.minecraft.sound.SoundEvents;
import net.minecraft.stat.Stats;
import net.minecraft.util.ActionResult;
import net.minecraft.util.Hand;
import net.minecraft.world.World;

public class TomahawkItem extends Item {
    private static final double TOMAHAWK_THROW_COST = 6.0; // Stamina cost to throw

    public TomahawkItem(Settings settings) {
        super(settings);
    }

    @Override
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ItemStack itemStack = user.getStackInHand(hand);

        // Check stamina on server side
        if (!world.isClient() && user instanceof ServerPlayerEntity serverPlayer) {
            double weightPenalty = ItemWeightSystem.calculateInventoryWeightPenalty(serverPlayer);
            double throwCost = TOMAHAWK_THROW_COST * (1.0 + weightPenalty * 0.3);

            if (!StaminaSystem.tryConsume(serverPlayer, throwCost)) {
                return ActionResult.FAIL; // Not enough stamina
            }
        }

        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_SNOWBALL_THROW, SoundCategory.NEUTRAL, 0.5f, 0.4f / (world.getRandom().nextFloat() * 0.4f + 0.8f));
        if (!world.isClient()) {
            TomahawkProjectileEntity tomahawk = new TomahawkProjectileEntity(world, user);
            tomahawk.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 0f);
            world.spawnEntity(tomahawk);
        }

        user.incrementStat(Stats.USED.getOrCreateStat(this));
        if (!user.getAbilities().creativeMode) {
            itemStack.decrement(1);
        }

        return ActionResult.SUCCESS;
    }
}
