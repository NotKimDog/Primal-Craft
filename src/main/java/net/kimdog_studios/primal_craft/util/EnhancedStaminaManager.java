...
public final class EnhancedStaminaManager {
    ...
    public static double getStamina(ServerPlayerEntity player) {
        ...
        PrimalCraft.LOGGER.trace("📊 [STAMINA_GET] {} current: {}/{}",
    ...
    public static double getMaxStamina(ServerPlayerEntity player) {
        ...
        PrimalCraft.LOGGER.trace("📊 [MAX_STAMINA] {} max: {}", player.getName().getString(), String.format("%.1f", maxStamina));
        PrimalCraft.LOGGER.trace("   ├─ Base: {}", MAX_STAMINA);
        PrimalCraft.LOGGER.trace("   ├─ Weight reduction: {}", String.format("%.1f", weightReduction));
        PrimalCraft.LOGGER.trace("   ├─ Fatigue: {} (reduction: {})", String.format("%.1f", fatigue), String.format("%.1f", fatigueReduction));
        PrimalCraft.LOGGER.trace("   └─ Final max: {}", String.format("%.1f", maxStamina));
    ...
    public static boolean tryConsume(ServerPlayerEntity player, double amount) {
        ...
        PrimalCraft.LOGGER.debug("💸 [CONSUME] Event #{} - {} consuming {}",
            consumes, player.getName().getString(), String.format("%.2f", amount));
        PrimalCraft.LOGGER.trace("   ├─ Current: {}/{}", String.format("%.1f", current), String.format("%.1f", max));
        PrimalCraft.LOGGER.trace("   ├─ Required: {}", String.format("%.2f", amount));

        if (current < amount) {
            PrimalCraft.LOGGER.trace("   └─ ✗ FAILED - Deficit: {}", String.format("%.2f", amount - current));
            return false;
        }
        ...
        PrimalCraft.LOGGER.trace("   └─ ✓ SUCCESS - {} → {}", String.format("%.1f", current), String.format("%.1f", newStamina));
    ...
    public static void restoreStamina(ServerPlayerEntity player, double amount) {
        ...
        PrimalCraft.LOGGER.debug("💚 [RESTORE] Event #{} - {} restoring {}",
            restorations, player.getName().getString(), String.format("%.2f", amount));
        PrimalCraft.LOGGER.trace("   ├─ Before: {}/{}", String.format("%.1f", current), String.format("%.1f", max));
        ...
        PrimalCraft.LOGGER.trace("   ├─ After: {}/{}", String.format("%.1f", newStamina), String.format("%.1f", max));
        PrimalCraft.LOGGER.trace("   ├─ Fatigue: {} → {} (reduced by {})",
            String.format("%.1f", fatigue), String.format("%.1f", newFatigue), String.format("%.1f", amount * 0.5));
        PrimalCraft.LOGGER.trace("   └─ ✓ Restoration applied");
    ...
    public static void tick(ServerPlayerEntity player) {
        ...
        if (ticks % 100 == 0) {
            PrimalCraft.LOGGER.trace("⏱️  [TICK] Tick #{} for {}", ticks, player.getName().getString());
        }
        ...
        if (ticks % 100 == 0) {
            PrimalCraft.LOGGER.trace("   ├─ Stamina regen: {} + {} = {}",
